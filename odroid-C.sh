...
## Odroid C Series MangoES ARM build
...
dd if=/dev/zero of=${basedir}/mangoes-$1-$2-odroid-C.img bs=1M count=3000 >> $mkImageMountLog 2>&1
...
cd /boot
/usr/sbin/update-initramfs -c -t -k $_VER
## rm -f /uinitram
EOF
...
cd ${basedir}
## FIXME this should be pulled in by something else like a package.
...
# dd if=${basedir}/u-boot/sd_fuse/bl1.bin.hardkernel of=${basedir}/mangoes-$1-$2-odroid-C.img bs=512 skip=1 seek=1 conv=notrunc >> $mkImageMountLog 2>$
...
rm -rf ${basedir}/kernel ${basedir}/bootp ${basedir}/root ${basedir}/mangoes-$architecture ${basedir}/bootp.tmp mangoes-armhf ${basedir}/root.tmp
#
## ${basedir}/web
## ${basedir}/u-boot
...
machineType=$(uname -m)
 ...
 then
   ...
   echo "</pre>" >> $compressLogFile
     ...
     mkdir -p /home/joseph/Work/MangoES/C1/
